...
const profileCopy: Record<LanguageCode, Record<string, string>> = {
  ...
  en: {
    ...
    freeDesc: 'Foundations & simple starter tools to explore.', free1: '✓ Lessons from Stages 1 and 2', free2: '✓ Limited AI access', free3: '✓ Basic checklists', freeAction: 'Request FREE plan', proDesc: 'Complete Academy, email automation & full AI toolbox.', pro1: '✓ Complete 99-stage learning path', pro2: '✓ GOM-MAR email automation', pro3: '✓ Ask GOM-MAR AI Mentor', pro4: '✓ All AI toolbox generators', proAction: 'Request PRO access', premiumDesc: 'For personal guidance, VIP training & VIP support.', premium1: '✓ Everything in PRO', premium2: '✓ Personal system audits', premium3: '✓ VIP one-to-one AI prompts', premiumAction: 'Request PREMIUM access', emailConsentTitle: 'Marketing emails', emailConsentDescription: 'I would like to receive Academy news, helpful tips, and offers by email. Consent is voluntary and can be withdrawn at any time.', emailConsentGranted: 'Consent granted', emailConsentNotGranted: 'No consent granted', emailConsentGrant: 'Grant consent', emailConsentWithdraw: 'Withdraw consent', emailConsentConfirmGrant: 'Would you like to receive marketing emails from GOM-MAR Academy?', emailConsentConfirmWithdraw: 'Would you like to withdraw your consent for marketing emails?', emailConsentSaved: 'Email preference saved.', emailConsentLoadError: 'Your email consent could not be loaded.', emailConsentSaveError: 'Your email consent could not be saved.', emailConsentAccountNotice: 'Important messages about your account and membership are not affected.'
  },
  pl: {
...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  useEffect(() => {
    ...
    void loadEmailConsent()
      .then((consent) => {
        if (!cancelled) setEmailConsentGranted(consent.granted);
      })
      .catch(() => {
  ...
  const handleEmailConsentChange = async () => {
    ...
    try {
      const consent = await saveEmailConsent(nextGranted);
      setEmailConsentGranted(consent.granted);
      setEmailConsentMessage(copy.emailConsentSaved);
      ...
      <section className="rounded-3xl border border-indigo-200 bg-white p-6 shadow-sm" aria-labelledby="email-consent-title">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${emailConsentGranted ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
              {emailConsentLoading ? '…' : emailConsentGranted ? copy.emailConsentGranted : copy.emailConsentNotGranted}
            </span>
          </div>
          <button
            type="button"
          ...
          >
            {emailConsentSaving ? '…' : emailConsentGranted ? copy.emailConsentWithdraw : copy.emailConsentGrant}
          </button>
        </div>
