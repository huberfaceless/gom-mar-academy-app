      ...
      <div className="space-y-8 animate-fadeIn">
        ...
        <div className="flex flex-col gap-6">
          {filteredStages.length === 0 ? (
            ...
            const isStageAccessible = isWithinTier;

            // Determine status type for badge
            let statusType: 'completed' | 'in_progress' | 'available' | 'locked' = 'locked';
            if (isStageComplete) {
              statusType = 'completed';
            } else if (isStageAccessible && (isStageInProgress || isCurrentActiveStage || stage.id === 1)) {
              statusType = 'in_progress';
            } else if (isStageAccessible) {
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  {stage.lessons.map((lesson, idx) => {
                    ...
                    const isNextActiveLesson = isStageAccessible && !isLessonDone && (
                      idx === 0 || completedTaskIdSet.has(stage.lessons[idx - 1]?.id)
                    );

                    if (isStageAccessible && isLessonDone) {
                      return (
                        <div 
                          key={lesson.id}
                  ...
                  <div className="prose prose-slate dark:prose-invert max-w-none text-slate-800 dark:text-slate-200 text-xs sm:text-sm leading-relaxed space-y-4 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl">
                    {currentLesson.learnContent.fullArticleGuide.split('\n\n').map((paragraph, idx) => {
                      if (paragraph.startsWith('### ')) {
                        ...
                        const localizedHeading = heading.trim().toUpperCase() === 'ZUSAMMENFASSUNG DER KERNPUNKTE:'
                          ? copy.summaryHeading
                          : heading;
                        return <h3 key={idx} className="text-base font-bold text-slate-900 dark:text-white mt-4 mb-2">{localizedHeading}</h3>;
                      }
                      if (paragraph.startsWith('#### ')) {
